...
export default function Home() {
  ...
  const featuredOffers = SELECTION_DU_MOMENT.map((slug) => managedOffers.find((offer) => offer.slug === slug)).filter((offer): offer is (typeof managedOffers)[number] => Boolean(offer));
  ...
  return (
    <main id="top" className={styles.page}>
      ...
      <section className={styles.featuredSection}><div className={styles.container}><div className={styles.featuredHeader}><div><span className={styles.kicker}>Sélection Parrainio</span><h2>Les offres <em>du moment.</em></h2></div></div><div className={styles.featuredGrid}>{featuredOffers.map((offer) => <Link href={`/offres/${offer.slug}`} className={styles.featuredCard} key={offer.slug}><div className={styles.featuredCardTop}><OfferLogo name={offer.name} logo={offer.logo} color={offer.color} logoLetter={offer.logoLetter} size={38} className={styles.featuredLogo} /><div className={styles.featuredInfo}><small>{offer.categoryGroup}</small><h3>{offer.name}</h3></div></div><OfferRewards offer={offer} compact /><span className={styles.featuredCta}>Voir l&apos;offre →</span></Link>)}</div></div></section>
